...
| 命令 | 作用 |
|------|------|
| `/generate 五年级下册 6` | 生成 6 个实验写入飞书 |
| `/list-tables` | 查看飞书现有数据表 |
| `/list-textbooks` | 查看本地教材文件 |

**B. 自然语言**（一句话触发）

直接敲下面这类句子，Claude Code 会自动识别并立刻执行，不会反问你细节：

- `把五年级下册的配套实验创建 6 个到飞书表格中`
- `为二年级下册生成 8 个实验`
- `看看飞书里现在有哪些数据表`
- `教材文件夹里有哪些教材？`

> 所有自动化规则都写在 `CLAUDE.md` 中，Claude Code 启动时自动读取，无需额外配置。
